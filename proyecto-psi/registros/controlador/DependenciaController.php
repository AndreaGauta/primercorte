...
class DependenciaController {
    ...
    function actualizar($dependencia){
        ...
        if($ps->rowCount() > 0){
            $mensaje = "SE actalizo correctamente la dependencia";
            $type = "success";
        }else{
            $mensaje = "No se puede actualizar la dependencia";
            $type = "error";
        }
        $this->conexion= null;
        $resultado = [
    ...
    function eliminar($dependencia)
    {
        ...
        if($ps->rowCount() > 0){
            $mensaje = "Se elimino correctamente la dependencia";
            $type = "success";
        }else{
            $mensaje = "No se puede eliminar la dependencia";
            $type = "error";
        }
